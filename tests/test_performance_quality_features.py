...
def test_robots_txt_404_fast_fail():
    ...
    response_404 = httpx.Response(
        status_code=404,
        request=httpx.Request("GET", "https://robots-404-domain.org/robots.txt"),
    )
    ...
    with pytest.raises(httpx.HTTPStatusError):
        http.get("https://robots-404-domain.org/robots.txt")
    ...
    assert checker.is_allowed("https://robots-404-domain.org/some-page") is True
...
def test_cooldown_escalation_and_blacklisting():
    ...
    try:
        ...
        state = http._cooldown_state_for(url)
        ...
        assert not state.is_blacklisted
        ...
        remove_from_blacklist(test_domain)
...
def test_robots_checker_netloc_caching():
    ...
    response_txt = httpx.Response(
        status_code=200,
        request=httpx.Request("GET", "https://robots-test-domain.org/robots.txt"),
        text="User-agent: *\nDisallow: /private",
    )
    ...
    # Check page 1
    assert checker.is_allowed("https://robots-test-domain.org/page1") is True
    # Check page 2 (same domain)
    assert checker.is_allowed("https://robots-test-domain.org/page2") is True
    # Check page 3 (private - disallowed)
    assert checker.is_allowed("https://robots-test-domain.org/private") is False
