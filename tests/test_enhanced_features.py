...
def test_json_api_link_discovery():
    ...
    assert "https://example.com/api?page=2" in [link["url"] for link in links]
    assert "https://example.com/related-page" in [link["url"] for link in links]
    assert "https://example.com/img.jpg" not in [link["url"] for link in links]
...
def test_http_client_no_retry_on_bypass_failure(monkeypatch):
    ...
    # Assert that stealth strategy was attempted
    assert call_count >= 1
...
def test_helium_fallback_triggers_when_crawl4ai_fails(monkeypatch):
    ...
    def mock_drission_fail(self, url, options):
        raise Exception("DrissionPage failed")

    def mock_helium_success(url):
