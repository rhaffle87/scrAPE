...
def mode_continuous_watchdog():
    ...
    cmd = [
        sys.executable,
        str(Path(__file__).parent / "monitor_agent.py"),
        "--keyword",
        keyword,
        "--interval",
        interval,
...
def print_mission_statement():
    print(
        f" {CLR_BOLD}{CLR_CYAN}═══════════════════ HIGH-EFFICIENCY FUEL PUMP FOR AI ═══════════════════{CLR_END}"
    )
    print(
        f"  scrAPE is essentially a {CLR_BOLD}high-efficiency fuel pump for AI{CLR_END}."
    )
    print("  By using a systematic layer-by-layer crawler (BFS) equipped with an")
    print("  AI-friendly scraper (Crawl4AI), it maps entire websites and converts")
    print("  messy internet pages into clean, structured text that AI models and LLMs")
    print("  can immediately understand.")
    print("  ")
    print("  Because scrAPE has built-in smart asset deduplication, concurrent")
    print("  downloading, and strict boundary controls, it solves the biggest")
    print("  headaches in data engineering:")
    print("  • Stops the crawler from getting lost on external sites")
    print("  • Cuts down massive storage costs by stripping out duplicate files")
    print("  • Gathers deep data at lightning speed")
    print("  ")
    print("  It is exactly what companies want for building custom AI knowledge bases,")
    print("  feeding vector databases for RAG applications, and running cost-effective")
    print("  AI data pipeline operations.")
    print(
        f" {CLR_BOLD}{CLR_CYAN}════════════════════════════════════════════════════════════════════════{CLR_END}\n"
    )
...
def select_completed_run():
    ...
    if not output_dir.exists():
        print(
            f" {CLR_FAIL}No output directory found. Please run a scrape first.{CLR_END}"
        )
        return None, None
    ...
    sub_choice = get_input(
        f"Select subject (1-{len(subjects)})", val_fn=validate_number
    )
...
def mode_create_dataset():
    ...
    print(
        "This utility groups and exports scraped files from a completed run into a single"
    )
    ...
    print(
        f"  1) {CLR_GREEN}{CLR_BOLD}Consolidated Flat{CLR_END} (All files in one folder, prefixed to prevent collisions)"
    )
    print(
        f"  2) {CLR_BLUE}{CLR_BOLD}Domain-Grouped{CLR_END} (Subfolders created for each source domain)"
    )
    print(
...
def mode_rag_ingest():
    ...
    print(
        f"{CLR_BOLD}{CLR_CYAN}─── Mode: Enterprise LLM RAG Ingestion Helper ───{CLR_END}\n"
    )
    print(
        "This utility processes crawled pages, text tokens, alt descriptions, and scores"
    )
    print(
        "into highly structured formats optimized for ingestion into Vector DBs/RAG pipelines.\n"
    )
    ...
    print(
        f"  1) {CLR_GREEN}{CLR_BOLD}Consolidated Markdown Document{CLR_END} (A single well-structured .md document of all findings)"
    )
    print(
        f"  2) {CLR_BLUE}{CLR_BOLD}Chunked Page-Level Documents{CLR_END} (Individual markdown files for each scraped page)"
    )
    print(
        f"  3) {CLR_CYAN}{CLR_BOLD}JSON-Lines (JSONL) format{CLR_END} (One JSON record per document, perfect for automated embedding pipelines)"
    )
    ...
    _metadata = data.get("run_metadata", {})
    ...
    if format_choice == "1":
        ...
        with open(md_file, "w", encoding="utf-8") as f:
            ...
            for idx, r in enumerate(reports, 1):
                f.write(
                    f"{idx}. [{r.get('url')}]({r.get('url')}) — Status: {r.get('status')} (Depth: {r.get('depth')})\n"
                )

            f.write("\n## Extracted Structured Media & Alt Text Metadata\n\n")
            f.write(
                "Below is the cleaned alt text, titles, and source URLs representing extracted knowledge.\n\n"
            )
            ...
            for idx, vid in enumerate(videos, 1):
                ...
                f.write(
                    f'- **Context Title/Label:** "{vid.get("page_title") or vid.get("type") or "N/A"}"\n'
                )
        ...
        for page_url, pdata in pages_dict.items():
            ...
            filename = (
                re.sub(r"[^a-zA-Z0-9]+", "_", parsed.netloc + parsed.path).strip("_")
                or "root"
            )
            ...
            with open(p_file, "w", encoding="utf-8") as f:
                f.write(
                    f"# Document: {pdata['report'].get('url') if pdata['report'] else page_url}\n\n"
                )
                if pdata["report"]:
                ...
                for idx, img in enumerate(pdata["images"], 1):
                    ...
                    f.write(
                        f"- **Description:** {img.get('alt_text') or 'No alt description available.'}\n"
                    )
...
def main():
    while True:
        ...
        print(f" {CLR_CYAN}▼ DATA COLLECTION & MONITORING{CLR_END}")
        print(
        ...
        print(f"\n {CLR_CYAN}▼ DATA EXPORT & AI INGESTION{CLR_END}")
        print(
